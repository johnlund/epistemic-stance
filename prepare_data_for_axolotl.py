...
def convert_to_sharegpt_format(row: pd.Series) -> dict:
    """Convert a single row to ShareGPT/ChatML format."""
...
def main():
    ...
    # Convert to JSONL
    print(f"Converting to ShareGPT format...")
    ...
    with open(output_path, 'w') as f:
        for _, row in df.iterrows():
            example = convert_to_sharegpt_format(row)
    ...
    # Print a sample
    print("\nSample output:")
    sample = convert_to_sharegpt_format(df.iloc[0])
